...
// Vulkan hello world triangle rendering demo with compute and queue family
// transition
...
const char* vertexShaderFilename = "./shaders/triangle.vert.spv";
const char* fragmentShaderFilename = "./shaders/triangle.frag.spv";
const char* computeShaderFilename = "./shaders/compute.comp.spv";
...
VkPhysicalDevice getPhysicalDevice(
    VkInstance instance);  // destroyed with instance
...
VkDeviceMemory initMemory(
    VkDevice device,
    VkPhysicalDeviceMemoryProperties physicalDeviceMemoryProperties, T resource,
...
VkDescriptorSetLayout initDescriptorSetLayout(
    VkDevice device, vector<VkDescriptorSetLayoutBinding> bindings);
void killDescriptorSetLayout(VkDevice device,
                             VkDescriptorSetLayout descriptorSetLayout);

VkDescriptorPool initDescriptorPool(VkDevice device, uint32_t maxSets,
                                    VkDescriptorType type, uint32_t maxDesc);
void killDescriptorPool(VkDevice device, VkDescriptorPool descriptorPool);

vector<VkDescriptorSet> acquireDescriptorSets(
...
VkPipelineLayout initPipelineLayout(
    VkDevice device, vector<VkDescriptorSetLayout> descriptorSetLayouts = {});
...
VkPipeline initComputePipeline(VkDevice device, VkPipelineLayout pipelineLayout,
                               VkShaderModule computeShader);
void killPipeline(VkDevice device, VkPipeline pipeline);

void setVertexData(VkDevice device, VkDeviceMemory memory,
                   vector<Vertex2D_ColorF_pack> vertices);
...
vector<VkCommandBuffer> acquireCommandBuffers(VkDevice device,
...
void recordBeginRenderPass(VkCommandBuffer commandBuffer,
                           ...
                           VkClearValue clearValue, uint32_t width,
                           uint32_t height);
void recordEndRenderPass(VkCommandBuffer commandBuffer);

void recordBindPipeline(VkCommandBuffer commandBuffer,
                        VkPipelineBindPoint bindPoint, VkPipeline pipeline);
void recordBindVertexBuffer(VkCommandBuffer commandBuffer,
                            const uint32_t vertexBufferBinding,
                            VkBuffer vertexBuffer);

void recordSetViewport(VkCommandBuffer commandBuffer, uint32_t width,
                       uint32_t height);
void recordSetScissor(VkCommandBuffer commandBuffer, uint32_t width,
                      uint32_t height);

void recordDraw(VkCommandBuffer commandBuffer,
                const vector<Vertex2D_ColorF_pack> vertices);

void recordImageBarrier(VkCommandBuffer commandBuffer, VkImage image,
...
void submitToQueue(VkQueue queue, VkCommandBuffer commandBuffer,
                   VkSemaphore waitS, VkPipelineStageFlags waitStage,
                   VkSemaphore signalS);
...
int main() try {
  ...
  vector<Vertex2D_ColorF_pack> triangle = {
      {/*rb*/ {0.5f * triangleSize, sqrtf(3.0f) * 0.25f * triangleSize},
       /*R*/ {1.0f, 0.0f, 0.0f}},
      {/* t*/ {0.0f, -sqrtf(3.0f) * 0.25f * triangleSize},
       /*G*/ {0.0f, 1.0f, 0.0f}},
      {/*lb*/ {-0.5f * triangleSize, sqrtf(3.0f) * 0.25f * triangleSize},
       /*B*/ {0.0f, 0.0f, 1.0f}}};

  vector<const char*> layers;
  if (::debugVulkan) layers.push_back("VK_LAYER_KHRONOS_validation");
  ...
  VkDebugReportCallbackEXT debug =
      ::debugVulkan ? initDebug(instance, ::debugAmount) : VK_NULL_HANDLE;
  ...
  VkDevice device = initDevice(physicalDevice, features, families, layers,
  ...
  if (surfaceCapabilities.currentExtent.width != ::windowWidth ||
      surfaceCapabilities.currentExtent.height != ::windowHeight) {
  ...
  VkSwapchainKHR swapchain =
      initSwapchain(physicalDevice, device, surface, surfaceFormat);
  ...
  vector<VkFramebuffer> framebuffers = initFramebuffers(
      device, renderPass, swapchainImageViews, ::windowWidth, ::windowHeight);

  VkShaderModule vertexShader =
      initShaderModule(device, ::vertexShaderFilename);
  VkShaderModule fragmentShader =
      initShaderModule(device, ::fragmentShaderFilename);
  VkPipelineLayout pipelineLayout = initPipelineLayout(device);
  VkPipeline pipeline = initPipeline(device, physicalDeviceProperties.limits,
  ...
  VkDescriptorSetLayoutBinding computeDescriptorSetLayoutBinding{
      computeImageBinding, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
      1,  // descriptorCount
      VK_SHADER_STAGE_COMPUTE_BIT,
      nullptr  // pImmutableSamplers -- ignored without samplers
  };
  VkDescriptorSetLayout computeDescriptorSetLayout =
      initDescriptorSetLayout(device, {computeDescriptorSetLayoutBinding});
  VkDescriptorPool computeDescriptorPool = initDescriptorPool(
      device, imageCount, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, imageCount);
  vector<VkDescriptorSet> computeDescriptorSets = acquireDescriptorSets(
      device, computeDescriptorPool,
      vector<VkDescriptorSetLayout>(imageCount, computeDescriptorSetLayout));

  VkShaderModule computeShader =
      initShaderModule(device, ::computeShaderFilename);
  VkPipelineLayout computePipelineLayout =
      initPipelineLayout(device, {computeDescriptorSetLayout});
  VkPipeline computePipeline =
      initComputePipeline(device, computePipelineLayout, computeShader);

  VkBuffer vertexBuffer = initBuffer(
      device, sizeof(decltype(triangle)::value_type) * triangle.size(),
      VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
  VkDeviceMemory vertexBufferMemory = initMemory<ResourceType::Buffer>(
      device, physicalDeviceMemoryProperties, vertexBuffer,
  ...
  VkCommandPool computeCommandPool =
      initCommandPool(device, computeQueueFamily);

  vector<VkCommandBuffer> commandBuffers =
      acquireCommandBuffers(device, commandPool, imageCount);
  for (size_t i = 0; i < commandBuffers.size(); ++i) {
    beginCommandBuffer(commandBuffers[i]);
    recordBeginRenderPass(commandBuffers[i], renderPass, framebuffers[i],
                          ::clearColor, ::windowWidth, ::windowHeight);

    recordBindPipeline(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                       pipeline);
    ...
    recordImageBarrier(commandBuffers[i], swapchainImages[i],
                       ...
                       VK_IMAGE_LAYOUT_GENERAL, queueFamily,
                       computeQueueFamily);
    endCommandBuffer(commandBuffers[i]);
  }

  vector<VkCommandBuffer> computeCommandBuffers =
      acquireCommandBuffers(device, computeCommandPool, imageCount);
  for (size_t i = 0; i < computeCommandBuffers.size(); ++i) {
    updateDescriptorSet(device, computeDescriptorSets[i], computeImageBinding,
                        swapchainImageViews[i], VK_IMAGE_LAYOUT_GENERAL);

    beginCommandBuffer(computeCommandBuffers[i]);
    recordBindPipeline(computeCommandBuffers[i], VK_PIPELINE_BIND_POINT_COMPUTE,
                       computePipeline);
    recordBindDescriptorSet(computeCommandBuffers[i],
                            VK_PIPELINE_BIND_POINT_COMPUTE,
                            computePipelineLayout, {computeDescriptorSets[i]});

    recordImageBarrier(computeCommandBuffers[i], swapchainImages[i],
    ...
    recordImageBarrier(computeCommandBuffers[i], swapchainImages[i],
                       ...
                       /*0*/ VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                       VK_ACCESS_SHADER_WRITE_BIT, 0, VK_IMAGE_LAYOUT_GENERAL,
                       VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, computeQueueFamily,
                       queueFamily);
    endCommandBuffer(computeCommandBuffers[i]);
  }

  vector<VkCommandBuffer> transferBackCommandBuffers =
      acquireCommandBuffers(device, commandPool, imageCount);
  for (size_t i = 0; i < computeCommandBuffers.size(); ++i) {
    beginCommandBuffer(transferBackCommandBuffers[i]);
    recordImageBarrier(transferBackCommandBuffers[i], swapchainImages[i],
                       /*0*/ VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    uint32_t nextSwapchainImageIndex =
        getNextImageIndex(device, swapchain, imageReadyS);
    submitToQueue(queue, commandBuffers[nextSwapchainImageIndex], imageReadyS,
                  VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, renderDoneS);
    submitToQueue(computeQueue, computeCommandBuffers[nextSwapchainImageIndex],
                  renderDoneS, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                  computeDoneS);
    submitToQueue(queue, transferBackCommandBuffers[nextSwapchainImageIndex],
                  computeDoneS, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
                  transferDoneS);
  ...
  cout << "Rendered " << frames << " frames in " << time_span.count()
       << " seconds. Average FPS is " << frames / time_span.count() << endl;
  ...
  cout << "ERROR: Terminated due to an uncaught VkResult exception: "
       << vkE.file << ":" << vkE.line << ":" << vkE.func << "() " << vkE.source
       << "() returned " << to_string(vkE.result) << endl;
} catch (const char* e) {
  cout << "ERROR: Terminated due to an uncaught exception: " << e << endl;
} catch (string e) {
  cout << "ERROR: Terminated due to an uncaught exception: " << e << endl;
} catch (std::exception e) {
  cout << "ERROR: Terminated due to an uncaught exception: " << e.what()
       << endl;
} catch (...) {
  cout << "ERROR: Terminated due to an unrecognized uncaught exception."
       << endl;
}
...
VkInstance initInstance(const vector<const char*> layers,
  ...
  VkInstanceCreateInfo instanceInfo{
      ...
      .pNext = nullptr,  // pNext for extensions use
      .flags = 0,        // flags - reserved for future use
      .pApplicationInfo =
  ...
  VkDebugReportCallbackCreateInfoEXT debugCreateInfo{
      VK_STRUCTURE_TYPE_DEBUG_REPORT_CREATE_INFO_EXT, nullptr, ::debugAmount,
      genericDebugCallback, nullptr};

  if (::debugVulkan) {
    instanceInfo.pNext =
        &debugCreateInfo;  // valid just for createInstance/destroyInstance
                           // execution
  }
...
void killInstance(VkInstance instance) { vkDestroyInstance(instance, nullptr); }
...
vector<VkPhysicalDevice> getPhysicalDevices(VkInstance instance) {
  ...
  do {
    ...
    errorCode = vkEnumeratePhysicalDevices(instance, &deviceCount, nullptr);
...
VkPhysicalDevice getPhysicalDevice(VkInstance instance) {
  ...
  if (devices.empty())
    throw "No VkPhysicalDevice found!";
  else if (devices.size() > 1) {
    cout << "WARNING: More than one VkPhysicalDevice found - just picking the "
            "first one.\n";
  }
...
uint32_t getDedicatedComputeQueueFamily(VkPhysicalDevice physDevice) {
  ...
  for (; qfi < qfps.size(); ++qfi) {
    // if( (qfps[qfi].queueFlags & VK_QUEUE_COMPUTE_BIT) &&
    // !(qfps[qfi].queueFlags & VK_QUEUE_GRAPHICS_BIT) ){ 	return qfi;
    // }
...
VkDevice initDevice(VkPhysicalDevice physDevice,
                    const VkPhysicalDeviceFeatures& features,
                    vector<uint32_t> queueFamilies, vector<const char*> layers,
                    vector<const char*> extensions) {
...
template <>
VkMemoryRequirements getMemoryRequirements<ResourceType::Buffer>(
...
VkDeviceMemory initMemory(
    VkDevice device,
    VkPhysicalDeviceMemoryProperties physicalDeviceMemoryProperties, T resource,
    VkMemoryPropertyFlags requiredFlags, VkMemoryPropertyFlags desiredFlags) {
  VkMemoryRequirements memoryRequirements =
    ...
    if (memoryRequirements.memoryTypeBits & 0x1) {
      if ((physicalDeviceMemoryProperties.memoryTypes[i].propertyFlags &
        ...
        if (!found) {
          memoryType = i;
          found = true;
        } else if (/*found &&*/ (physicalDeviceMemoryProperties.memoryTypes[i]
                                     .propertyFlags &
...
vector<VkSurfaceFormatKHR> getSurfaceFormats(VkPhysicalDevice physicalDevice,
  ...
  do {
    ...
    errorCode = vkGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface,
                                                     &formatCount, nullptr);
    RESULT_HANDLER(errorCode, "vkGetPhysicalDeviceSurfaceFormatsKHR");

    formats.resize(formatCount);
    errorCode = vkGetPhysicalDeviceSurfaceFormatsKHR(
...
VkSurfaceFormatKHR getSurfaceFormat(VkPhysicalDevice physicalDevice,
  ...
  for (auto f : formats) {
    if (f.format == /*VK_FORMAT_B8G8R8A8_SRGB*/ VK_FORMAT_B8G8R8A8_UNORM &&
...
vector<VkPresentModeKHR> getSurfacePresentModes(VkPhysicalDevice physicalDevice,
  ...
  do {
    ...
    errorCode = vkGetPhysicalDeviceSurfacePresentModesKHR(
...
VkPresentModeKHR getSurfacePresentMode(VkPhysicalDevice physicalDevice,
                                       VkSurfaceKHR surface) {
  vector<VkPresentModeKHR> modes =
      getSurfacePresentModes(physicalDevice, surface);
...
VkSwapchainKHR initSwapchain(VkPhysicalDevice physicalDevice, VkDevice device,
  ...
  uint32_t minImageCount =
      std::min<uint32_t>(capabilities.maxImageCount,
                         std::max<uint32_t>(2, capabilities.minImageCount));
  ...
  VkSwapchainCreateInfoKHR swapchainInfo{
      ...
      1,
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
          VK_IMAGE_USAGE_STORAGE_BIT,  // VkImage usage flags
      VK_SHARING_MODE_EXCLUSIVE,
...
vector<VkDescriptorSet> acquireDescriptorSets(
  ...
  VkDescriptorSetAllocateInfo descriptorSetInfo{
      ...
      nullptr,  // pNext
      descriptorPool, static_cast<uint32_t>(descriptorSetLayouts.size()),
      descriptorSetLayouts.data()};

  vector<VkDescriptorSet> descriptorSets(descriptorSetLayouts.size());
  VkResult errorCode = vkAllocateDescriptorSets(device, &descriptorSetInfo,
                                                descriptorSets.data());
  RESULT_HANDLER(errorCode, "vkAllocateDescriptorSets");
...
void updateDescriptorSet(VkDevice device, VkDescriptorSet descriptorSet,
                         uint32_t binding, VkImageView imageView,
                         VkImageLayout expectedLayout) {
  VkDescriptorImageInfo imageInfo{VK_NULL_HANDLE,  // sampler
                                  imageView, expectedLayout};
...
void recordBindDescriptorSet(VkCommandBuffer commandBuffer,
  ...
  vkCmdBindDescriptorSets(commandBuffer, bindPoint, pipelineLayout,
                          ...
                          descriptorSets.data(), 0, nullptr  // dynamic offsets
  );
}
...
VkShaderModule initShaderModule(VkDevice device, string filename) {
  ...
  if (!ifs.is_open())
    throw string("SPIR-V shader file failed to open: ") + strerror(errno);
  vector<char> shaderCode(
      (istreambuf_iterator<char>(ifs)),
      istreambuf_iterator<char>() /* EOS */);  // Most Vexing Parse

  if (shaderCode.empty() ||
      shaderCode.size() % 4 !=
          0 /* per spec; % sizeof(uint32_t) presumably */) {
...
VkPipelineLayout initPipelineLayout(
  ...
  VkPipelineLayoutCreateInfo pipelineLayoutInfo{
      ...
      static_cast<uint32_t>(
          descriptorSetLayouts.size()),  // descriptorSetLayout count
      descriptorSetLayouts.data(),
...
VkPipeline initPipeline(VkDevice device, VkPhysicalDeviceLimits limits,
  ...
  vector<VkPipelineShaderStageCreateInfo> shaderStageStates = {
  ...
  if (offsetof(Vertex2D_ColorF_pack, color) >
  ...
  VkVertexInputAttributeDescription positionInputAttributeDescription{
      positionLocation, vertexBufferBinding, VK_FORMAT_R32G32_SFLOAT,
  ...
  vector<VkDynamicState> dynamicObjects = {VK_DYNAMIC_STATE_VIEWPORT,
                                           VK_DYNAMIC_STATE_SCISSOR};
  VkPipelineDynamicStateCreateInfo dynamicState{
      VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO,
      nullptr,  // pNext
      0,        // flags - reserved for future use
      static_cast<uint32_t>(dynamicObjects.size()), dynamicObjects.data()};

  VkGraphicsPipelineCreateInfo pipelineInfo{
      VK_STRUCTURE_TYPE_GRAPHICS_PIPELINE_CREATE_INFO,
      nullptr,  // pNext
      0,        // flags - e.g. disable optimization
      static_cast<uint32_t>(shaderStageStates.size()),  // shader stages count -
                                                        // vertex and fragment
      shaderStageStates.data(), &vertexInputState, &inputAssemblyState,
      nullptr,  // tesselation
...
void recordDraw(VkCommandBuffer commandBuffer,
  ...
  vkCmdDraw(commandBuffer, static_cast<uint32_t>(vertices.size()),
            1 /*instance count*/, 0 /*first vertex*/, 0 /*first instance*/);
}
...
void recordImageBarrier(VkCommandBuffer commandBuffer, VkImage image,
                        ...
                        VkPipelineStageFlags dstStage, VkAccessFlags srcAccess,
                        VkAccessFlags dstAccess, VkImageLayout oldLayout,
                        VkImageLayout newLayout, uint32_t oldQueueFamily,
  ...
  vkCmdPipelineBarrier(commandBuffer, srcStage, dstStage,
                       0 /*or VK_DEPENDENCY_BY_REGION_BIT*/, 0, nullptr, 0,
...
void submitToQueue(VkQueue queue, VkCommandBuffer commandBuffer,
  ...
  VkResult errorCode = vkQueueSubmit(queue, 1 /*submit count*/, &submit,
...
void present(VkQueue queue, VkSwapchainKHR swapchain,
  ...
  VkResult errorCode = vkQueuePresentKHR(queue, &presentInfo);
